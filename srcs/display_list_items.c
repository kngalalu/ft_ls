...
/*
** function to get file ACL
*/
static char get_file_acl(char path[PATH_MAX])
{
    char buf[101];

    /* Use llistxattr() to list extended attributes without following symlinks */
    if (llistxattr(path, buf, sizeof(buf)) > 0)
        return ('@');
#ifdef ACL_SUPPORT
    {
        acl_t tmp = acl_get_file(path, ACL_TYPE_ACCESS);
        if (tmp != NULL)
        {
            acl_free(tmp);
            return ('+');
...
** name is not displayed if file type is a link
*/
