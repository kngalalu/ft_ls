#include "ft_ls.h"
#include <sys/ioctl.h>  // Include for struct winsize

/*
** Define t_index if not already defined
*/
typedef struct {
    int x;
    int y;
...
/* Function prototype for missing function */
...
{
	t_index			maxlen;
	struct winsize	ts;  // Replaced struct ttysize with struct winsize
	t_index			i;
	t_file			*tmp_lst;
	int				total;
	...
	i.x = (!(flags & 128)) ? ts.ws_col / (maxlen.x + maxlen.y) : 1; // Changed ts_cols -> ws_col
	i.y = 0;
	tmp_lst = lst;
	while (tmp_lst)
	{
